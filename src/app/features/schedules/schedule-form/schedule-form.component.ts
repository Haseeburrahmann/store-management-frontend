...
stores: Store[] = [];
employees: Employee[] = [];
availableStores: Store[] = []; // Stores the user can manage
availableEmployees: Employee[] = []; // Employees the user can schedule
...
ngOnInit(): void {
  ...
  // Load reference data based on role
  this.loadUserData();
}

loadUserData(): void {
  if (this.userRole === 'manager') {
    // Managers can only edit schedules for their assigned stores
    this.loadManagerStores();
  } else if (this.userRole === 'admin') {
    // Admins can edit schedules for all stores
    this.loadAllStores();
    ...
    this.error = 'You do not have permission to create or edit schedules';
    this.loading = false;
  }
}

loadManagerStores(): void {
  const currentUser = this.authService.currentUser;
  if (!currentUser) {
  ...
  // Get stores that this manager is assigned to
  this.storeService.getStoresByManager(currentUser._id).subscribe({
    next: (stores) => {
      this.stores = stores;
      this.availableStores = stores;
      this.currentUserStoreIds = stores.map(store => store._id);
      
      // Load employees for these stores
...
loadStoreEmployees(): void {
  if (this.currentUserStoreIds.length === 0) {
    this.availableEmployees = [];
    return;
  }
  
  // For each store, load its employees
  const employeePromises = this.currentUserStoreIds.map(storeId => 
    this.employeeService.getEmployeesByStore(storeId).toPromise()
  );
  
  Promise.all(employeePromises)
    .then(employeeLists => {
      // Flatten and deduplicate the employee lists
      const allEmployees = employeeLists.flat();
      const uniqueEmployeeMap = new Map();
      
      allEmployees.forEach(employee => {
        if (employee && employee._id) {
          uniqueEmployeeMap.set(employee._id, employee);
        }
      });
      
      this.employees = Array.from(uniqueEmployeeMap.values());
...
loadSchedule(): void {
  this.hoursService.getSchedule(this.scheduleId).subscribe({
    next: (schedule) => {
      ...
      if (this.userRole === 'manager' && 
          !this.currentUserStoreIds.includes(schedule.store_id)) {
        this.error = 'You do not have permission to edit this schedule';
        this.loading = false;
        return;
      }
...
isFormValid(): boolean {
  ...
  for (const shift of this.shifts) {
    ...
    // Validate start time is before end time (with accommodation for overnight shifts)
...
saveSchedule(): void {
  ...
  // For managers, ensure they can only edit schedules for their stores
  if (this.userRole === 'manager' && !this.currentUserStoreIds.includes(this.schedule.store_id as string)) {
    this.error = 'You do not have permission to create schedules for this store';
    return;
  }
